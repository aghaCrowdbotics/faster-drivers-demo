...
export default function Payments({ onAdd }) {
  ...
  const handleSubscribe = () => {
    api.addSubscription({
      ...
      if (ok) {
        api.fetchUserApi();
        history.push('/restaurant/settings/subscription');
        toast({
          title: 'Success',
  ...
  return <Flex p={4} direction={'column'} justifyContent={'space-between'} h={'100%'} w={'100%'}>
    ...
    {location?.state &&
      <Flex marginTop={'10px'} justifyContent={'center'} gap={2}>
        <Button w={'80%'} bg={'primary.600'} color={'white'} h={14} disabled={pending || activePaymentId === null}
