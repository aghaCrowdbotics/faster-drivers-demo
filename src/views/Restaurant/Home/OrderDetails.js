import { Button, Avatar, Flex, Image, Spinner, Text, useToast } from "@chakra-ui/react";
...
export default function OrderDetails() {
  ...
  const { id, user, dishes, status, total, tip, taxes, fees, sub_total, special_instructions, driver } = activeOrder
      ...
      return (
        <Flex direction={'column'} boxShadow={'lg'} gap={2}>
          ...
          <Flex direction={'column'} padding={4}>
            ...
            {special_instructions && <Text fontSize={14}>{special_instructions}</Text>}
            {driver &&
              <Flex marginTop={4} direction={'column'}>
                <Text fontWeight={600}>Driver</Text>
                <Flex marginTop={3} justifyContent={'flex-start'}>
                  <Avatar src={driver?.driver?.photo} w={'70px'} h={'70px'} objectFit={'cover'} />
                  <Flex direction={'column'} padding='0px 15px'>
                    {driver?.name && <Text>{driver?.name}</Text>}
                    {driver?.driver?.rating && <Text>{`${driver?.driver?.rating_count}/${driver?.driver?.rating}`}</Text>}
                  </Flex>
                </Flex>
              </Flex>
            }
